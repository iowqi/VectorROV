...
N         = 4800;           % propeller speed [RPM]
D         = 0.066;         % propeller diameter [m]
   
THRUST    = 50;         % required thrust [N]
Vs        = 1.5;         % ship velocity [m/s]
Dhub      = 0.030;        % hub diameter [m] (must be greater than 0.15*D) 
...
Rduct      = D/2+0.00001;          % duct radius [m]
Cduct      = D/2+0.00001;          % duct chord length [m]
...
XR         = linspace(0,1.0,10);    % radius / propeller radius
XCoD = -0.2*log(XR)+0.18; % chord / diameter
...
t0oc0      = 0.15*XR; % max section thickness / chord
skew0      = -10*XR; % skew [deg]
rake0      = -XR*0.08; % rake / diameter
